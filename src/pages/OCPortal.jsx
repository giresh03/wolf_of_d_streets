...
const OCPortal = () => {
    ...
    try {
      ...
      if (useFirebase) {
        ...
        await setDoc(doc(db, 'oc_transactions', transactionId), {
          ...
          amount: amount,
          reason: reason || (amount > 0 ? 'Correct fake news identification' : 'Incorrect fake news identification'),
        ...
        localStorage.setItem(`teamData_${selectedTeam.id}`, JSON.stringify(teamData));
      }

      alert(`✅ Successfully ${amount > 0 ? 'added' : 'deducted'} ₹${Math.abs(amount)} ${amount > 0 ? 'to' : 'from'} ${selectedTeam.teamName}`);
  ...
  const handleDeductMoney = async () => {
    ...
    if (!window.confirm(`Deduct ₹${deductAmount} from ${selectedTeam.teamName}?`)) {
      return;
    }
    ...
    try {
      ...
      alert(`✅ Successfully deducted ₹${deductAmount} from ${selectedTeam.teamName}`);
      ...
      <div className="container mx-auto px-4 py-4 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
          <div>
            ...
            <p className="text-green-200 text-sm sm:text-base">Organizing Committee Dashboard</p>
            {!useFirebase && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 sm:p-6 shadow-xl">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">👥 Select Team</h2>
              ...
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-white/10 border-b border-white/20">
                  <tr>
                    ...
                    <th className="px-3 py-3 text-white font-semibold text-xs sm:text-sm">Capital</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTeams.length === 0 ? (
                    ...
                    filteredTeams.map((team, index) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-3 py-3 text-gray-300 text-xs sm:text-sm">{index + 1}</td>
                          <td className="px-3 py-3 text-white font-medium text-xs sm:text-sm">
                            {team.teamName || 'Unnamed Team'}
                            {teamStatuses[team.teamName]?.attended && (
                              <span className="ml-2 text-xs bg-green-500 text-white px-2 py-0.5 rounded">✓</span>
                            )}
                          </td>
            ...
            <div className="mt-4 p-3 bg-white/5 rounded-lg">
              <p className="text-gray-300 text-xs">
                <span className="bg-green-500 text-white px-2 py-0.5 rounded mr-2">✓</span>
                Present teams
              </p>
              <p className="text-gray-400 text-xs mt-2">
                📝 Note: Team credentials are available in the "Credentials & Attendance" tab
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 sm:p-6 shadow-xl">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">💰 Manage Money</h2>
            
            {selectedTeam ? (
              <div className="space-y-4">
                {/* Selected Team Info */}
                <div className="bg-green-500/20 border border-green-500 rounded-lg p-4">
                  <h3 className="text-white font-bold text-lg mb-2">Selected Team:</h3>
                  <p className="text-2xl font-bold text-green-300 mb-3">{selectedTeam.teamName}</p>
                  
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/10 rounded-lg p-3">
                      <p className="text-gray-300 text-xs mb-1">Current Capital</p>
                      <p className="text-xl font-bold text-green-400">
                        ₹{selectedTeam.currentCapital?.toFixed(2) || '0'}
                      </p>
                    </div>
                    <div className="bg-white/10 rounded-lg p-3">
                      <p className="text-gray-300 text-xs mb-1">Portfolio Value</p>
                      <p className="text-xl font-bold text-blue-400">
                        ₹{((selectedTeam.currentCapital || 0) + 
                          (selectedTeam.stock1Shares || 0) * (selectedTeam.stock1Price || 0) +
                          (selectedTeam.stock2Shares || 0) * (selectedTeam.stock2Price || 0)
                        ).toFixed(2)}
                      </p>
                    </div>
                  </div>
                ...
                {/* Amount Input */}
                <div>
                  <label className="block text-white mb-2 font-medium text-sm">Amount (₹)</label>
                  <input
                    type="number"
                    value={amount}
                ...
                {/* Reason Input */}
                <div>
                  <label className="block text-white mb-2 font-medium text-sm">Reason (Optional)</label>
                  <input
                    type="text"
                    value={reason}
                ...
                <div className="bg-blue-500/20 border border-blue-500 rounded-lg p-4 mt-4">
                  <h4 className="text-blue-200 font-medium mb-2 text-sm">📋 Round 2 - Fake News Rules:</h4>
              ...
              <div className="flex flex-col items-center justify-center h-96 text-center">
                ...
                <p className="text-gray-300 text-lg">Select a team from the left panel</p>
                <p className="text-gray-400 text-sm mt-2">Click on any team to manage their money</p>
              </div>
            )}
          </div>
        </div>
