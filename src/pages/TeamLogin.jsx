...
const TeamLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Generate team ID from name (slug)
      const teamId = teamName.toLowerCase().replace(/\s+/g, "_");

      if (db) {
        // Initialize team in Firestore if missing
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white/10 backdrop-blur-md rounded-2xl p-6 sm:p-8 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4 sm:space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 sm:px-4 py-2 sm:py-3 rounded-lg bg-white/20 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="mt-4 sm:mt-6 flex justify-center gap-4 text-center">
          <a href="/oc" className="text-green-300 hover:text-green-200 text-xs sm:text-sm">
            OC Portal →
          </a>
          <a href="/admin" className="text-purple-300 hover:text-purple-200 text-xs sm:text-sm">
            Admin Panel →
          </a>
        </div>
        ...
        <div className="mt-6 sm:mt-8 bg-blue-500/20 border border-blue-500 rounded-lg p-3 sm:p-4">
          <h3 className="text-blue-200 font-medium mb-2 text-sm sm:text-base">
            📋 Instructions:
          </h3>
        ...
        <div className="mt-4 text-center">
          <p className="text-gray-400 text-xs">Lost your credentials? Contact event organizers</p>
        </div>
      </div>
    </div>
  );
